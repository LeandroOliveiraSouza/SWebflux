...
public class UserController {
    ...
    public Mono<ResponseEntity<UserDTO>> insert(@RequestBody UserDTO dto, UriComponentsBuilder builder) {
        return service.insert(dto).map(x -> ResponseEntity.created(builder.path("/users/{id}").buildAndExpand(x.getId()).toUri()).body(x));
    }

    @PutMapping(value = "/{id}")
    public Mono<ResponseEntity<UserDTO>> update(@PathVariable String id, @RequestBody UserDTO dto) {
        return service.update(id, dto).map(x -> ResponseEntity.ok().body(x));
    }
}
